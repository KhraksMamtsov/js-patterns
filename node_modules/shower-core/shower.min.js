...
!function ( a ) {
	var b, c = {
		...
		var l = {
			...
			var a, b = {};
			for ( var c in m ) {
				m.hasOwnProperty(c) && (a = m[c], (b[a.decl.state] || (b[a.decl.state] = [])).push(c));
			}
			return b
		}, v  = function () {
			n = !1, w()
			...
			for ( var g, h, i = [], j = function ( a, b ) {
				if (b){
					return void d(null, b);
				}
				if (!--e){
					for ( var c, f = [], g = 0; c = i[g++]; ) {
						f.push(c.exports);
					}
					d(f)
				}
			...
			if (b.state === c.RESOLVED){
				return void e(b.exports);
			}
			if (b.state === c.IN_RESOLVING){
				return void(l.trackCircularDependencies && j(b, d) ? e(null, g(b, d))
					: b.dependents.push(e));
			}
			if (b.dependents.push(e), b.prev && !l.allowMultipleDeclarations){
				return void A(b, i(b));
			}
			...
			for ( var e, f = 0; e = a.dependents[f++]; ) {
				e(d);
			}
			a.dependents = b
		}, A  = function ( a, b ) {
			a.state = c.NOT_RESOLVED;
			for ( var d, e = 0; d = a.dependents[e++]; ) {
				d(null, b);
			}
			a.dependents = []
		};
		...
		return Error(b ? 'Module "' + b.name + '": can\'t resolve dependence "' + a + '"' : 'Required module "' + a +
		                                                                                    "\" can't be resolved")
	}, g     = function ( a, b ) {
		for ( var c, d = [], e = 0; c = b[e++]; ) {
			d.push(c.name);
		}
		...
		if (a.postMessage && !a.opera){
			...
			if (e){
				...
				return a.addEventListener ? a.addEventListener("message", h, !0) : a.attachEvent("onmessage",
				                                                                                 h), function ( b ) {
					c(b) && a.postMessage(g, "*")
				}
			}
	...
	a.shower = {
		...
		a.shower.modules.require("shower.defaultOptions", function ( d ) {
			var e                                                                                        = a.hasOwnProperty("showerOptions"), f = a.shower.options, g = f.shower_selector ||
			                                                                     d.container_selector, h = document.querySelector(
				g), i                                                                                    = b.bind(this, h), j                                                               = "undefined" != typeof f.auto_init ? f.auto_init : !0;
			if (!h){
				throw new Error("Shower element with selector " + g + " not found.");
			}
	...
	d(e.prototype, {
		on                : function ( a, b, c, d ) {
			...
			if (d = d || 0, "string" == typeof a){
				this._addListener(a, b, c, d);
			}
			else {
				for ( var e = 0, f = a.length; f > e; e++ ) {
					this._addListener(a[e], b, c, d);
				}
			}
			return this
		},
		off               : function ( a, b, c, d ) {
			if (d = d || 0, "string" == typeof a){
				this._removeListener(a, b, c, d);
			}
			else {
				for ( var e = 0, f = a.length; f > e; e++ ) {
					this._removeListener(a[e], b, c, d);
				}
			}
			return this
		},
		...
		emit              : function ( a, b ) {
			...
			(c = this.createEventObject(a, b, this._context)), c.isPropagationStopped() || (d.hasOwnProperty(a) &&
			                                                                                this._callListeners(d[a],
			                                                                                                    c), this._parent &&
		...
		_removeListener   : function ( a, b, c, d ) {
			...
			if (f){
				for ( var g = -1, h = 0, i = f.length; i > h; h++ ) {
					e = f[h], e.callback == b && e.context == c &&
					          e.priority == d && (g = h);
				}
	...
	b(c.prototype, {
		...
		offAll          : function () {
			for ( var a = 0, b = this._listeners.length; b > a; a += 3 ) {
				this.events.off(this._listeners[a],
				                this._listeners[a + 1],
				                this._listeners[a + 2]);
			}
		...
		_removeListener : function ( a, b, c ) {
			for ( var d = this._listeners.indexOf(a, 0); -1 != d; ) {
				this._listeners[d + 1] == b &&
				this._listeners[d + 2] == c &&
				(this._listeners.splice(d, 3), this.events.off(a, b,
				                                               c)), d =
					this._listeners.indexOf(a, d)
			}
		}
	}), a(c)
}), shower.modules.define("Plugins", ["Emitter", "util.extend"], function ( a, b, c ) {
	function d( a ) {
		this.events = new b({ context : this }), this._showerGlobal = a, this._showerInstances =
			a.getInited(), this._plugins = {}, this._instances = [], a.events.on("init", this._onShowerInit, this)
	}
	...
	c(d.prototype, {
		...
		add             : function ( a, b ) {
			if (this._plugins.hasOwnProperty(a)){
				throw new Error("Plugin " + a + " already exist.");
			}
		...
		_instanceFor    : function ( a ) {
			for ( var b in this._plugins ) {
				this._plugins.hasOwnProperty(b) && this._instance(this._plugins[b], a)
			}
		},
...
}), shower.modules.define("Options", ["Emitter", "options.Monitor", "util.Store", "util.extend", "util.inherit"],
	...
	function c( a ) {
		this._options = a, this._optionsEvents =
			a.events.group().on(["set", "unset"], this._onOptionsChange, this), this._fieldsHanders = {}
	}
	...
	b(c.prototype, {
		...
		add              : function ( a, b, c ) {
			if (Array.prototype.isArray.call(null, a)){
				var d = a;
				for ( var e in d ) {
					d.hasOwnProperty(e) && this._addHandler(e, b, c)
				}
			}
			else {
				this._addHandler(a, b, c);
			}
			return this
		},
		remove           : function ( a, b, c ) {
			if (Array.prototype.isArray.call(null, a)){
				var d = a;
				for ( var e in d ) {
					d.hasOwnProperty(e) && this._remodeHandler(e, b, c)
				}
			}
			else {
				this._remodeHandler(a, b, c);
			}
			return this
		},
		getOptions       : function () {
		...
		_addHandler      : function ( a, b, c ) {
			...
			this._fieldsHanders.hasOwnProperty(fieldName) ? this._fieldsHanders[fieldName].push(d)
				: this._fieldsHanders[fieldName] = [d]
		},
		_remodeHandler   : function ( a, b, c ) {
			if (!this._fieldsHanders.hasOwnProperty(a)){
				throw new Error("Remove undefined handler for " + a + " field");
			}
			var d = this._fieldsHanders[a], e = d.filter(function ( a ) {
				return a.callback === b && a.context === c
			})[0];
			if (!hander){
				throw new Error("Hanlder for " + a + " not found.");
			}
			d.splice(d.indexOf(e, 1))
		}
	}), a(c)
}), shower.modules.define("Shower", ["Emitter",
                                     "Options",
                                     "shower.global",
                                     ...
                                     "shower.Location",
                                     "shower.slidesParser",
                                     "util.extend"], function ( a, b, c, d, e, f, g, h, i, j ) {
	function k( a, i ) {
		i = i || {}, this.events = new b({ context : this }), this.options = new c({}, e, i);
		var j = a || this.options.get("container_selector");
		"string" == typeof j && (j = document.querySelector(j)), this.player = new g(this), this.container =
			new f(this, j), this._slides = [], this._isHotkeysOn = !0, this._liveRegion =
			null, this._initSlides(), this._initLiveRegion(), this.options.get("debug_mode") &&
		                                                      (document.body.classList.add(this.options.get(
			                                                      "debug_mode_classname")), console.info(
			                                                      "Debug mode on")), this.options.get("hotkeys") ||
		                                                                             this.disableHotkeys(), this.location =
			new h(this), d.events.emit("notify", { shower : this }), this._playerListeners =
			this.player.events.group().on("activate", this._onPlayerSlideActivate, this)
	}

	j(k.prototype, {
		destroy                : function () {
			this.events.emit(
				"destroy"), this.location.destroy(), this.container.destroy(), this.player.destroy(), this._slides = []
		},
		add                    : function ( a ) {
		...
		_initSlides            : function () {
			var a = this.options.get("slides_parser") || i, b = a(this.container.getElement(),
		...
		_initLiveRegion        : function () {
			...
			a.setAttribute("role", "region"), a.setAttribute("aria-live", "assertive"), a.setAttribute("aria-relevant",
			                                                                                           "additions"), a.setAttribute(
				"aria-label", "Slide Content: Auto-updating"), a.className = "region", document.body.appendChild(
				a), this._liveRegion = a
		}
	}), a(k)
	...
	d(e.prototype, {
		init                 : function () {
			var a = document.body.classList, b = this._shower.options, c = b.get("mode_full_classname"), d = b.get(
				"mode_list_classname");
		...
		enterSlideMode       : function () {
			...
			return a.remove(b.get("mode_list_classname")), a.add(
				b.get("mode_full_classname")), document.body.setAttribute("role", "application"), this._applyTransform(
				this._getTransformScale()), this._isSlideMode = !0, this.events.emit("slidemodeenter"), this
		},
		exitSlideMode        : function () {
			var a = document.body.classList, b = this._shower.options;
			return a.remove(b.get("mode_full_classname")), a.add(
				b.get("mode_list_classname")), document.body.removeAttribute("role",
			                                                                 "application"), this._applyTransform(
				"none"), this._isSlideMode = !1, this.scrollToCurrentSlide(), this.events.emit("slidemodeexit"), this
		},
		...
		_setupListeners      : function () {
			this._showerListeners = this._shower.events.group().on("slideadd", this._onSlideAdd, this).on("slideremove",
			                                                                                              this._onSlideRemove,
			                                                                                              this), window.addEventListener(
				"resize", c(this, "_onResize")), document.addEventListener("keydown", c(this, "_onKeyDown"))
		},
		_clearListeners      : function () {
			this._showerListeners.offAll(), window.removeEventListener("resize", c(this,
			                                                                       "_onResize")), document.removeEventListener(
				"keydown", c(this, "_onKeyDown"))
		},
		_getTransformScale   : function () {
			var a = Math.max(document.body.clientWidth / window.innerWidth,
		...
		_onKeyDown           : function ( a ) {
			if (this._shower.isHotkeysEnabled()){
				switch ( a.which ) {
					case 13:
						if (a.preventDefault(), this.isSlideMode()){
...
}), shower.modules.define("shower.Location", ["util.SessionStore", "util.bound", "util.extend"],
                          function ( a, b, c, d ) {
	                          function e( a ) {
		                          this._shower = a;
		                          var c        = a.options.get("sessionstore_key") + "-shower.Location";
		                          this.state = new b(c, { isSlideMode : !1 }), this._showerListeners =
			                          null, this._playerListeners = null, this._documentTitle =
			                          document.title, this._popStateProcess = null, this._setupListeners(), this._init()
	                          }
	                          ...
	                          d(e.prototype, {
		                          ...
		                          _init                       : function () {
			                          var a, b = this._shower, c = window.location.hash.substr(1), d = b.options.get(
				                          "mode_full_classname");
			                          window.location.hash = "", (this.state.get("isSlideMode") ||
			                                                      document.body.classList.contains(d)) &&
			                                                     b.container.enterSlideMode(), "" !== c && (a =
				                          this._getSlideById(c), b.player.go(
				                          "undefined" != typeof a.index ? a.index : 0))
		                          },
		                          _setupListeners             : function () {
			                          var a = this._shower;
			                          this._playerListeners =
				                          a.player.events.group().on("activate", this._onSlideActivate,
				                                                     this), this._containerListener =
				                          a.container.events.group().on(["slidemodeenter", "slidemodeexit"],
				                                                        this._onContainerSlideModeChange,
				                                                        this), window.addEventListener("popstate",
			                                                                                           c(this,
			                                                                                             "_onPopstate"))
		                          },
		                          _clearListeners             : function () {
			                          window.removeEventListener("popstate", c(this,
			                                                                   "_onPopstate")), this._playerListeners.offAll(), this._containerListener.offAll()
		                          },
		                          _getSlideById               : function ( a ) {
			                          for ( var b, c, d = this._shower.getSlides(), e = d.length - 1; e >=
			                                                                                          0; e-- ) {
				                          if (d[e].getId() ===
				                              a){
					                          b = d[e], c = e;
		                          ...
		                          _onPopstate                 : function () {
			                          var a, b                                  = this._shower, c                = window.location.hash.substr(
				                          1), d = b.player.getCurrentSlide(), e = b.player.getCurrentSlideIndex();
			                          this._isSlideMode() && -1 === e ? b.player.go(0) : -1 === e &&
		                          ...
		                          _setTitle                   : function () {
			                          ...
			                          if (b && c){
				                          ...
				                          this._documentTitle !== a && (document.title = this._documentTitle)
			                          }
		                          }
	                          }), a(e)
                          }), shower.modules.define("shower.Player", ["Emitter", "util.bound", "util.extend"],
                                                    function ( a, b, c, d ) {
	                                                    function e( a ) {
		                                                    this.events = new b({
			                                                    context : this,
			                                                    parent  : a.events
		                                                    }), this._shower = a, this._showerListeners =
			                                                    null, this._playerListeners =
			                                                    null, this._currentSlideNumber =
			                                                    -1, this._currentSlide = null, this.init()
	                                                    }

	                                                    d(e.prototype, {
		                                                    init                       : function () {
			                                                    this._showerListeners =
				                                                    this._shower.events.group().on("slideadd",
				                                                                                   this._onSlideAdd,
				                                                                                   this).on(
					                                                    "slideremove", this._onSlideRemove, this).on(
					                                                    "slidemodeenter",
					                                                    this._onContainerSlideModeEnter,
					                                                    this), this._playerListeners =
				                                                    this.events.group().on("prev", this._onPrev,
				                                                                           this).on("next",
				                                                                                    this._onNext,
		                                                    ...
		                                                    destroy                    : function () {
			                                                    this._showerListeners.offAll(), this._playerListeners.offAll(), document.removeEventListener(
				                                                    "keydown",
				                                                    c(this, "_onKeyDown")), this._currentSlide =
				                                                    null, this._currentSlideNumber =
				                                                    null, this._shower = null
		                                                    },
		                                                    next                       : function () {
		                                                    ...
		                                                    go                         : function ( a ) {
			                                                    "number" != typeof a &&
			                                                    (a = this._shower.getSlideIndex(a));
			                                                    var b = this._shower.getSlidesCount(), c = this._currentSlide;
			                                                    return a != this._currentSlideNumber && b > a &&
			                                                           a >= 0 &&
			                                                           (c && c.isActive() && c.deactivate(), c =
				                                                           this._shower.get(a), this._currentSlide =
				                                                           c, this._currentSlideNumber =
				                                                           a, c.isActive() ||
			                                                                  c.activate(), this.events.emit("activate",
			                                                                                                 {
				                                                                                                 index : a,
				                                                                                                 slide : c
			                                                                                                 })), this
		                                                    ...
		                                                    _onSlideActivate           : function ( a ) {
			                                                    var b = a.get("slide"), c = this._shower.getSlideIndex(
				                                                    b);
			                                                    this.go(c)
		                                                    },
		                                                    _onKeyDown                 : function ( a ) {
			                                                    if (this._shower.isHotkeysEnabled() &&
			                                                        !/^(?:button|input|select|textarea)$/i.test(
				                                                        a.target.tagName)){
				                                                    switch ( this.events.emit(
					                                                    "keydown", { event : a }), a.which ) {
					                                                    case 33:
					                                                    case 38:
					                                                    case 37:
					                                                    case 72:
					                                                    ...
					                                                    case 35:
						                                                    ...
						                                                    break;
					                                                    case 32:
						                                                    this._shower.container.isSlideMode() &&
						                                                    (a.shiftKey ? this.prev() : this.next())
				                                                    }
			                                                    }
		                                                    },
		                                                    _onContainerSlideModeEnter : function () {
			                                                    this._currentSlide || this.go(0)
                                                    ...
                                                    }), shower.modules.define("shower.defaultOptions",
                                                                              function ( a, b ) {
	                                                                              a({
                                                                              ...
                                                                              }), shower.modules.define(
	"shower.slidesParser", ["Slide"], function ( a, b ) {
		function c( a, c ) {
		...
		a(c)
	}), shower.modules.define("Slide", ["shower.defaultOptions",
                                        "Emitter",
                                        "Options",
                                        "slide.Layout",
                                        "slide.layoutFactory",
                                        "util.Store",
                                        "util.extend"], function ( a, b, c, d, e, f, g, h ) {
	function i( a, b, e ) {
	...
	h(i.prototype, {
		init            : function () {
			this.layout = "string" == typeof this._content ? new f.createLayout({ content : this._content })
...
}), shower.modules.define("slide.Layout", ["Options", "shower.defaultOptions", "Emitter", "util.bound", "util.extend"],
                          function ( a, b, c, d, e, f ) {
	                          function g( a, e ) {
		                          this.options = new b({
		                          ...
		                          }, e), this.events = new d, this._element = a, this._parent =
			                          null, this._parentElement = null, this.init()
	                          }
	                          ...
	                          f(g.prototype, {
		                          ...
		                          setParent          : function ( a ) {
			                          this._parent != a && (this._clearListeners(), this._parent = a, this._parent &&
			                          this._setupListeners(), this.events.emit("parentchange", { parent : a }))
		                          },
		                          getParent          : function () {
			                          return this._parent
		                          },
		                          setParentElement   : function ( a ) {
			                          ...
			                          (this._parentElement = a, a.appendChild(this._element), this.events.emit(
				                          "parentelementchange", { parentElement : a }))
		                          },
		                          ...
		                          setTitle           : function ( a ) {
			                          var b                            = this.options.get(
				                          "title_element_selector"), c = this._element.querySelector(b);
			                          c ? c.innerHTML = a : (c = document.createElement(b), c.innerHTML =
				                          a, this._element.insertBefore(c, this._element.firstChild))
		                          },
		                          getTitle           : function () {
			                          var a                            = this.options.get(
				                          "title_element_selector"), b = this._element.querySelector(a);
		                          ...
		                          _setupListeners    : function () {
			                          this._slideListeners =
				                          this._parent.events.group().on("activate", this._onSlideActivate, this).on(
					                          "deactivate", this._onSlideDeactivate,
					                          this), this._element.addEventListener("click", e(this, "_onSlideClick"),
			                                                                        !1)
		                          },
		                          _clearListeners    : function () {
			                          this._slideListeners &&
			                          this._slideListeners.offAll(), this._element.removeEventListener("click", e(this,
			                                                                                                      "_onSlideClick"))
		                          },
		                          ...
		                          _onSlideDeactivate : function () {
			                          ...
			                          a.remove(this.options.get("active_classname")), a.add(
				                          this.options.get("visited_classname"))
		                          },
		                          _onSlideClick      : function () {
			                          this.events.emit("click")
		                          }
	                          }), a(g)
                          }), shower.modules.define("slide.layoutFactory", ["slide.Layout", "util.extend"],
                                                    function ( a, b, c ) {
	                                                    var d = {};
	                                                    c(d, {
		                                                    ...
		                                                    _createElement : function ( a ) {
			                                                    ...
			                                                    return b.innerHTML = a.content, b.classList.add(
				                                                    a.contentType), b
		                                                    }
	                                                    }), a(d)
                                                    }), shower.modules.define("util.SessionStore",
                                                                              ["util.Store", "util.inherit"],
                                                                              function ( a, b, c ) {
	                                                                              function d( a, b ) {
		                                                                              this._storageKey = a;
		                                                                              var c            = this._loadFromStorage() ||
		                                                                                                 b;
		                                                                              d["super"].constructor.call(this,
		                                                                                                          c)
	                                                                              }

	                                                                              c(d, b, {
		                                                                              set              : function ( a, b ) {
			                                                                              d["super"].set.call(this, a,
			                                                                                                  b), this._saveToStorage()
		                                                                              },
		                                                                              unset            : function ( a ) {
			                                                                              d["super"].unset.call(this,
			                                                                                                    a), this._saveToStorage()
		                                                                              },
                                                                                                        ...
                                                                                                        function ( a, b ) {
	                                                                                                        function c( a ) {
		                                                                                                        this._data =
			                                                                                                        a ||
			                                                                                                        {};
		                                                                                                        for ( var c = 1, d = arguments.length; d >
		                                                                                                                                               c; c++ ) {
			                                                                                                        b(
				                                                                                                        this._data,
				                                                                                                        arguments[c] ||
	                                                                                                        ...
	                                                                                                        b(c.prototype,
	                                                                                                          {
		                                                                                                          get     : function ( a, b ) {
			                                                                                                          return this._data.hasOwnProperty(
				                                                                                                          a)
				                                                                                                          ? this._data[a]
				                                                                                                          : b
		                                                                                                          },
	...
	var c = function ( a, c, d ) {
		return a.prototype = Object.create(c.prototype), a.prototype.constructor = a, a["super"] =
			c.prototype, a["super"].constructor = c, d && b(a.prototype, d), a.prototype
	};
	a(c)
}), shower.modules.define("shower-next", ["shower", "Emitter", "util.extend"], function ( a, b, c, d ) {
	function e( a, b ) {
		b = b || {}, this.events = new c({ context : this }), this._shower = a, this._elementsSelector =
			b.selector || g, this._elements = [], this._innerComplete = 0, this._setupListeners(), -1 !=
		                                                                                           this._shower.player.getCurrentSlideIndex() &&
	...
	d(e.prototype, {
		destroy                   : function () {
			this._clearListeners(), this._elements = null, this._elementsSelector = null, this._innerComplete =
				null, this._shower = null
		},
		next                      : function () {
		...
		_setupListeners           : function () {
			var a = this._shower;
			this._showerListeners = a.events.group().on("destroy", this.destroy, this), this._playerListeners =
				a.player.events.group().on("activate", this._onSlideActivate, this).on("next", this._onNext, this).on(
					"prev", this._onPrev, this);
			var c = b.plugins.get(f, a);
			c ? this._setupTimerPluginListener(c) : this._pluginsListeners =
				b.plugins.events.group().on("add", function ( a ) {
		...
		_clearListeners           : function () {
			this._showerListeners.offAll(), this._playerListeners.offAll(), this._pluginsListeners &&
	...
	function c( a, b ) {
		b = b || {}, this._shower = a, this._playerListeners = null, this._element = null, this._elementSelector =
			b.selector || ".progress";
		var c = this._shower.container.getElement();
		this._element = c.querySelector(this._elementSelector),
		this._element &&
		(this._setupListeners(), this._element.setAttribute("role", "progressbar"), this._element.setAttribute(
			"aria-valuemin", "0"), this._element.setAttribute("aria-valuemax", "100"), this.updateProgress())
	}
	...
	b(c.prototype, {
		...
		updateProgress  : function () {
			var a = this._shower.getSlidesCount(), b = this._shower.player.getCurrentSlideIndex(), c = 100 / (a - 1) *
			                                                                                           b;
			this._element &&
			(this._element.style.width = c.toFixed(2) + "%", this._element.setAttribute("aria-valuenow",
			                                                                            c.toFixed()), this._element.setAttribute(
				"aria-valuetext", "Slideshow Progress: " + c.toFixed() + "%"))
		},
		_setupListeners : function () {
			var a = this._shower;
			this._showerListeners = a.events.group().on("destroy", this.destroy, this), this._playerListeners =
				a.player.events.group().on("activate", this._onSlideChange, this)
		},
		_clearListeners : function () {
			this._showerListeners && this._showerListeners.offAll(), this._playerListeners &&
	...
	function e( a ) {
		this.events = new c, this._shower = a, this._timer = null, this._showerListeners = null, this._playerListeners =
			null, this._pluginsListeners = null, this._setupListeners()
	}
	...
	d(e.prototype, {
		...
		_setupListeners  : function () {
			var a = this._shower;
			this.events.on("next", this._onNext, this), this._showerListeners =
				a.events.group().on("destroy", this.destroy, this), this._playerListeners =
				a.player.events.group().on("keydown", this._clearTimer, this).on("activate", this._onSlideActivate,
				                                                                 this), this._nextPlugin =
				b.plugins.get(f, a), this._nextPlugin ||
			                         (this._pluginsListeners = a.plugins.events.group().on("pluginadd", function ( b ) {
				                         b.get("name") === f &&
				                         (this._nextPlugin = a.plugins.get(f), this._pluginsListeners.offAll())
		...
		_onSlideActivate : function () {
			...
			if (this._shower.container.isSlideMode() && a.state.get("visited") < 2){
				...
				b && /^(\d{1,2}:)?\d{1,3}$/.test(b) &&
				(-1 !== b.indexOf(":") ? (b = b.split(":"), b = 1e3 * (60 * parseInt(b[0], 10) + parseInt(b[1], 10)))
					: b = 1e3 * parseInt(b, 10), 0 !== b && this._initTimer(b))
			}
		},
		_initTimer       : function ( a ) {
			var b = this.events, c = (this._shower, this._nextPlugin);
			c && c.getLength() && c.getLength() != c.getComplete() && (a /= c.getLength() + 1), this._timer =
	...
	b(c.prototype, {
		...
		_setupListeners           : function () {
			var a = this._shower;
			this._showerListeners = a.events.group().on("add", this._onSlideAdd, this), this._bindedTouchStart =
				this._onTouchStart.bind(this), this._bindedTouchMove =
				this._onTouchMove.bind(this), this._shower.getSlides().forEach(this._addTouchStartListener,
			                                                                   this), document.addEventListener(
				"touchmove", this._bindedTouchMove, !0)
		},
		_clearListeners           : function () {
			this._showerListeners.offAll(), this._shower.getSlides().forEach(this._removeTouchStartListener,
			                                                                 this), document.removeEventListener(
				"touchmove", this._bindedTouchMove, !1)
		},
		_onSlideAdd               : function ( a ) {
			var b = a.get("slide");
		...
		_onTouchStart             : function ( a ) {
			var b, c = this._shower, d = c.container.isSlideMode(), e = a.target, f = this._getSlideByElement(
				a.currentTarget);
			f && (d && !this._isInteractiveElement(e) &&
			(a.preventDefault(), b = a.touches[0].pageX, b > window.innerWidth / 2 ? c.player.next()
				: c.player.prev()), d || f.activate())
		},
